...
// -------------------------------------------------------------------------------------------------
// API GET call to get information from the users Table
...
// Delete user event registration
app.delete('/deleteUserEvent/:userEventId', async (request, response) => {
    const userEventId = request.params.userEventId; // Capture the userEventId from the URL
    ...
    try {
        ...
        return response.status(204).send(); // Send a 204 No Content response
    } catch (error) {
        console.error('Error executing query', error);
        response.status(500).json({ error: 'Internal Server Error' });
    }
